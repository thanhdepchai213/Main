...
BringEnemy = function(Mon)
    ...
    pcall(function()
        ...
        -- Gom đúng 2 con: Mon + tối đa 1 mob cùng tên
        local gatheredCount = 1
        
        for _, v in ipairs(workspace.Enemies:GetChildren()) do
            if gatheredCount >= 2 then break end
            if v ~= Mon then
...
local Fluent, SaveManager, InterfaceManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/tv7775464-netizen/Maruhub/refs/heads/main/GET_modified_image_126048951669906_ALL.lua"))()
...
-- Neji Hub Auto Raid core merged into OKHub
_G.Raiding = false
local RaidIslands = {"Island 1", "Island 2", "Island 3", "Island 4", "Island 5"}
local RaidIslandRadius = 750
local RaidLocations = workspace:WaitForChild("_WorldOrigin"):WaitForChild("Locations")
local RaidLastTP = 0
local RaidTPCooldown = 0.55
local RaidLockedIsland = nil
local RaidCurrentIslandIndex = 1
local RaidThread = nil

local function OKRaidTP(cf)
    if tick() - RaidLastTP < RaidTPCooldown + math.random(0, 200) / 1000 then return end
    RaidLastTP = tick()
    pcall(function()
        if _tp then
            _tp(cf)
        elseif plr and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            plr.Character.HumanoidRootPart.CFrame = cf
        end
    end)
end

local function OKRaidInRaid()
    local pg = plr and plr:FindFirstChild("PlayerGui")
    local main = pg and pg:FindFirstChild("Main")
    local top = main and main:FindFirstChild("TopHUDList")
    local timer = top and top:FindFirstChild("RaidTimer")
    return timer and timer.Visible == true
end

local function OKRaidHasMob(pos)
    local enemies = workspace:FindFirstChild("Enemies")
    if not enemies then return false end
    for _, mob in ipairs(enemies:GetChildren()) do
        local hum = mob:FindFirstChildOfClass("Humanoid")
        local root = mob:FindFirstChild("HumanoidRootPart")
        if hum and root and hum.Health > 0.1 and (root.Position - pos).Magnitude <= RaidIslandRadius + 80 then
            return true
        end
    end
    return false
...
local function OKRaidFindBestIsland()
    for i = #RaidIslands, 1, -1 do
        local island = RaidLocations:FindFirstChild(RaidIslands[i])
        if island and OKRaidHasMob(island.Position) then return i end
    end
    for i = #RaidIslands, 1, -1 do
        if RaidLocations:FindFirstChild(RaidIslands[i]) then return i end
    end
    return 1
end

local function OKRaidEnsureIsland()
    if RaidLockedIsland and RaidLockedIsland.Parent then return RaidLockedIsland end
    RaidCurrentIslandIndex = OKRaidFindBestIsland()
    RaidLockedIsland = RaidLocations:FindFirstChild(RaidIslands[RaidCurrentIslandIndex])
    return RaidLockedIsland
end

local function OKRaidKillIsland(pos)
    local enemies = workspace:FindFirstChild("Enemies")
    if not enemies then return false end
    local hit = false
    for _, mob in ipairs(enemies:GetChildren()) do
        if not _G.Raiding or not OKRaidInRaid() then break end
        local hum = mob:FindFirstChildOfClass("Humanoid")
        local root = mob:FindFirstChild("HumanoidRootPart")
        if hum and root and hum.Health > 0.1 and (root.Position - pos).Magnitude <= RaidIslandRadius + 100 then
            hit = true
            local started = tick()
            while _G.Raiding and mob.Parent and hum.Health > 0.1 and OKRaidInRaid() do
                if (root.Position - pos).Magnitude > RaidIslandRadius + 120 then break end
                OKRaidTP(root.CFrame * CFrame.new(0, 5, -7))
                if Attack and Attack.Kill then
                    pcall(function() Attack.Kill(mob, true) end)
                end
                task.wait(0.12 + math.random(0, 80) / 1000)
                if tick() - started > 12 then break end
            end
        end
    end
    return hit
...
local function OKRaidStart()
    ...
    RaidThread = task.spawn(function()
        while task.wait(0.3) do
            ...
            local island = OKRaidEnsureIsland()
            if not island then continue end
            local dist = (hrp.Position - island.Position).Magnitude
            if dist > RaidIslandRadius + 150 then
                OKRaidTP(island.CFrame * CFrame.new(0, 60, 0))
                task.wait(0.8)
                continue
            end
            local hasMob = false
            if RaidCurrentIslandIndex == 4 then
                _G.KillAuraFull = true
                while _G.Raiding and OKRaidInRaid() and OKRaidHasMob(island.Position) do task.wait(0.5) end
                _G.KillAuraFull = false
            else
                hasMob = OKRaidKillIsland(island.Position)
            end
            if not hasMob or not OKRaidHasMob(island.Position) then
                RaidLockedIsland = nil
                RaidCurrentIslandIndex = OKRaidFindBestIsland()
                local nextIsland = RaidLocations:FindFirstChild(RaidIslands[RaidCurrentIslandIndex])
                if nextIsland then
                    OKRaidTP(nextIsland.CFrame * CFrame.new(0, 60, 0))
                    task.wait(1)
                end
            end
        end
...
Raiding = Tabs.Raids:AddToggle({
    Name = "Auto Raid [Neji] + Next Island",
    Description = "Auto raid theo logic Neji Hub",
    Default = false,
    Callback = function(Value)
        if Value then
            OKRaidStart()
...
-- ===== Long Ngu Ngu UI / Performance Overlay =====
--[[
...
imageLabel.Image = "rbxassetid://89947810111304"
...
alternateTextLabel.Text = "LongHiHi"
