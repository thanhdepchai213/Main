do
  ...
  Brazier3 = false
  Sec = 0.1
  -- Priority profile: Farming + Sea Event get tighter polling; other features keep normal timing.
  SX_FarmBoost = true
  SX_SeaBoost = true
  SX_FastPoll = 0.035
  ClickState = 0
...
BringEnemy = function(Mon)
    ...
    pcall(function()
        ...
        -- Gom đúng 2 con: con mục tiêu + chỉ 1 con cùng loại gần nhất.
        local broughtCount = 1
        for _, v in ipairs(workspace.Enemies:GetChildren()) do
            if broughtCount >= 2 then break end
            if v ~= Mon then
...
local function CreateSkibidiTopTag()
    local player = Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    local head = character:FindFirstChild("Head")
    if not head then return end
    local old = head:FindFirstChild("SkibidiXHubTop")
    if old then old:Destroy() end
    local billboard = Instance.new("BillboardGui")
    billboard.Name = "SkibidiXHubTop"
    ...
    label.Text = "Phongzuima hub By @NgHaiPhong"
    label.TextSize = 18
...
-- Central Tween movement: 120 studs/s
local TweenService = game:GetService("TweenService")
...
local Window = Fluent:CreateWindow({
    Title = "Phongzuima hub",
    SubTitle = "by @NguyenHaiPhong",
...
local Tabs = {
    Discord = wrapTab(Window:AddTab({ Title = "Discord", Icon = "rbxassetid://73132811772878" })),
    Info = wrapTab(Window:AddTab({ Title = " Status", Icon = "Info" })),
    Main = wrapTab(Window:AddTab({ Title = " Farming", Icon = "rbxassetid://7733960981" })),
    Settings = wrapTab(Window:AddTab({ Title = " Setting", Icon = "rbxassetid://7734053495" })),
    Fish = wrapTab(Window:AddTab({ Title = " Fishing", Icon = "rbxassetid://127664059821666" })),
    Quests = wrapTab(Window:AddTab({ Title = " Quest And Item", Icon = "rbxassetid://13075622619" })),
    SeaEvent = wrapTab(Window:AddTab({ Title = " Sea Event", Icon = "waves" })),
    Race = wrapTab(Window:AddTab({ Title = " Mirage And Race", Icon = "rbxassetid://11162889532" })),
    Prehistoric = wrapTab(Window:AddTab({ Title = " Volcano Event", Icon = "tent" })),
    Esp = wrapTab(Window:AddTab({ Title = " Stats And Esp", Icon = "rbxassetid://7040410130" })),
    Raids = wrapTab(Window:AddTab({ Title = " Fruit And Raid", Icon = "rbxassetid://11155986081" })),
    Combat = wrapTab(Window:AddTab({ Title = " Local Player", Icon = "rbxassetid://13075651575" })),
    Travel = wrapTab(Window:AddTab({ Title = " Teleport", Icon = "locate" })),
    Shop = wrapTab(Window:AddTab({ Title = " Shopping", Icon = "rbxassetid://6031265976" })),
    Misc = wrapTab(Window:AddTab({ Title = " Miscellaneous", Icon = "rbxassetid://10709783577" })),
    Config = wrapTab(Window:AddTab({ Title = " Config", Icon = "save" }))
}
...
pcall(function()
    if SaveManager then
        ...
        if SaveManager.BuildConfigSection then SaveManager:BuildConfigSection(Tabs.Config) end
    end
end)
...
pcall(function()
    if InterfaceManager then
        ...
        if InterfaceManager.BuildInterfaceSection then InterfaceManager:BuildInterfaceSection(Tabs.Config) end
    end
end)
...
Tabs.Discord:AddSection("Discord")
...
Tabs.Main:AddToggle({
    Name = "Farm Boost",
    Description = "Faster farm polling and target refresh",
    Default = true,
    Callback = function(Value)
        SX_FarmBoost = Value
    end
...
task.spawn(function()
    while task.wait(SX_FarmBoost and SX_FastPoll or Sec) do
        if _G.Level then
            pcall(function()
                ...
                elseif inSub or level < 2600 then
                    ...
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        if v.Name == enemyName and Attack.Alive(v) then
                            foundMob = true
                            repeat
                                task.wait(SX_FarmBoost and SX_FastPoll or Sec)
        ...
        for i,v in pairs(workspace.Enemies:GetChildren()) do
          if v:FindFirstChild("Humanoid") or v:FindFirstChild("HumanoidRootPart") then
            if v.Humanoid.Health > 0 then
              repeat task.wait(SX_FarmBoost and SX_FastPoll or Sec) Attack.Kill(v,_G.AutoFarmNear) until not _G.AutoFarmNear or not v.Parent or v.Humanoid.Health <= 0
            end
          end
        end
      end
...
task.spawn(function()
    while task.wait(SX_FarmBoost and SX_FastPoll or 0.2) do
...
Tabs.SeaEvent:AddToggle({
    Name = "Sea Event Boost",
    Description = "Faster sea-event scanning and response",
    Default = true,
    Callback = function(Value)
        SX_SeaBoost = Value
    end
...
spawn(function()
  while task.wait(SX_SeaBoost and SX_FastPoll or Sec) do
      ...
      pcall(function()        
        ...
        elseif myBoat and not(CheckShark()and _G.Shark or CheckTerrorShark()and _G.TerrorShark or CheckFishCrew()and _G.MobCrew or CheckPiranha()and _G.Piranha)and not(CheckEnemiesBoat()and _G.FishBoat)and not(CheckSeaBeast()and _G.SeaBeast1)and not(_G.PGB and CheckPirateGrandBrigade())and not(_G.HCM and CheckHauntedCrew())and not(_G.Leviathan1 and CheckLeviathan())then
          if plr.Character.Humanoid.Sit == false then
            ...
            repeat task.wait(SX_SeaBoost and SX_FastPoll or Sec) 
...
spawn(function()
  while task.wait(SX_SeaBoost and SX_FastPoll or Sec) do
    pcall(function()	
      if _G.Shark then local a={"Shark"}if CheckShark()then for b,c in pairs(workspace.Enemies:GetChildren())do if table.find(a,c.Name)then if Attack.Alive(c)then repeat task.wait(SX_SeaBoost and SX_FastPoll or Sec) Attack.Kill(c,_G.Shark)until _G.Shark==false or not c.Parent or c.Humanoid.Health<=0 end end end end end
      if _G.TerrorShark then local a={"Terrorshark"}if CheckTerrorShark()then for b,c in pairs(workspace.Enemies:GetChildren())do if table.find(a,c.Name)then if Attack.Alive(c)then repeat task.wait(SX_SeaBoost and SX_FastPoll or Sec) Attack.KillSea(c,_G.TerrorShark)until _G.TerrorShark==false or not c.Parent or c.Humanoid.Health<=0 end end end end end
      if _G.Piranha then local a={"Piranha"}if CheckPiranha()then for b,c in pairs(workspace.Enemies:GetChildren())do if table.find(a,c.Name)then if Attack.Alive(c)then repeat task.wait(SX_SeaBoost and SX_FastPoll or Sec) Attack.Kill(c,_G.Piranha)until _G.Piranha==false or not c.Parent or c.Humanoid.Health<=0 end end end end end
...
-- Tween 120 override for common movement entry points.
local __Original_tp = _tp
local __Original_topos = topos
